...
def best_threshold(y: np.ndarray, p: np.ndarray, objective: str = "balanced_accuracy") -> float:
    ...
    Подбирает порог по тем же данным, на которых его потом применяют, поэтому BA и F1 при
    таком пороге завышены. Годится только для калибровки на данных, отдельных от оценки.
    """
